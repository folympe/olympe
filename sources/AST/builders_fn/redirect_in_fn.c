...
static t_node	*create_redirect_in_node(t_token *token)
{
	t_node	*node;
...
static int	add_redirect_in(t_token *token, t_node **tree)
{
	t_node	*node;
	t_node	*curr_node;

	node = create_redirect_in_node(token);
	if (!node)
		return (0);
	if (!*tree)
	{
...
t_ast_res	redirect_in_fn(t_token *token, t_node **tree)
{
	...
	if (!token->next || token->next->type != TOKEN_WORD)
		return (res);
	res.success = add_redirect_in(token, tree);
	res.tree = *tree;
