...
/*   Updated: 2026/09/06 09:56:55 by jiasmoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static int	erase_var(t_expand_token **token_node, char *str, int index)
{
	char	*new_str;
	int		i;
	int		j;
...
static char	*erase_and_replace(t_expand_token **token_node, char *str,
...
char	*replace_str_heredoc(char *str, char *var_value, int index)
{
	...
	if (!var_value)
	{
		...
		token.str = str;
		current = &token;
		if (erase_var(&current, str, index))
			return (NULL);
		return (token.str);
	}
	old = str;
